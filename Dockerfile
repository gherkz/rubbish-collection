...
ENV DJANGO_SETTINGS_MODULE=rubbish_collection_project.settings
...
ENV DJANGO_SETTINGS_MODULE=rubbish_collection_project.settings.developer
...
CMD ["exec gunicorn \
  ...
  --access-logfile=- \
  --capture-output \
  rubbish_collection_project.wsgi" \
]
